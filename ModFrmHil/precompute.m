...
import !"Algebra/AlgAss/ideals-jv.m" : ColonInternal, 
                                            has_element_of_norm,
                                            twisted_trace_form,
...
"The short values are (sample only): ", ords_short_vals[i]; 
  ...
  */
   ...
   if not small_prime then  // NP large relative to h ==> quicker to run through ideal classes
        ...
        for i in inds do 
          ...
          if not IsDefined(ridls_colonZBs, <j,i>) then
            ...
            if use_Jprime then 
              ...
              // also recording the ideal
              ridls_colons[<j,i>] := ridls[i] * JJ *b^(-1);
            else
              ...
              // recording the ideals themselves
              ridls_colons[<j,i>] := icolonj;
            end if;
          end if;
        end for; // i in inds
          ...
          for u in pos_units_mod_norms[i] do
            if not can_find_nbrs[i] then
              ...
              if debug then
                ...
                assert bool eq bool_old;
                if bool then 
                  assert #elts eq #elts_old;
                end if;
              end if; 
            end if;
            if bool then 
      ...
      OH`RightIdealClasses[record_idx]`rids_colons := ridls_colons; // update cache (note: this might use a lot of memory!)
...
function get_tps_for_rids(OA, rids, p : rows:=0, support:=[])
  ...
  if not assigned rec`rids_colons then
    rec`rids_colons := AssociativeArray(CartesianProduct([1..h],[1..h]));
  end if;
  if not assigned rec`tps then 
