...
fibration $Y\to\bP^1$ whose only non-reduced fibre is a double fibre supported
on the vanishing locus of the weight-$2$ cusp form.
\end{theorem}

\begin{proof}
Combine Propositions~\ref{prop:invariants}, \ref{prop:P2}, \ref{prop:P3},
Theorem~\ref{thm:main}, Corollary~\ref{cor:canring} and
Proposition~\ref{prop:fibration}.
\end{proof}

\paragraph{Status of the fibre configuration.}
The base $\bP^1$, its coordinate $t=\eta/\Om^2$, the multiple fibre
$2\{\Om=0\}$ and the total discriminant degree $\deg\Delta_{\mathrm{ell}}=
e(Y)=24$ are determined. The full list of singular fibres (Kodaira types)
requires the Weierstrass model of $f$, equivalently the branch data of the
degree-$4$ map $Y\to\bP^2$ given by $M_2$ (note $(2L)^2=4$, since
$\dim M_k\sim\tfrac12 k^2$ gives $L^2=1$); this is the subject of ongoing
computation.
