...
Working over the reduction of the saved model at a prime $p$, adjoining the
single equation $F_1-t_0F_2$ cuts $Y$ down to the (one-dimensional) fibre
$Y_{t_0}$; eliminating the nine generators of weight $\ge6$ and then one of the
two remaining weight-$2$ coordinates yields a plane model $G(x_2,x_4)=0$
birational to $Y_{t_0}$ (one relation is linear in the eliminated coordinate, so
the projection is birational), of geometric genus $1$. A point is found by
factoring $G(x_2^0,\,\cdot\,)$, and $\mathrm{EllipticCurve}(\,\cdot\,,\mathrm{pt})$
returns a Weierstrass model and hence $j(t_0)$.

Sampling $j(t_0)$ and reconstructing by Cauchy (rational-function) interpolation
gives $j(t)=N(t)/D(t)$ with $\deg N=\deg D=24$; fitting on $50$ nodes and
verifying on the remaining and on further fresh nodes fixes the degree
unambiguously (a fit on $48$ nodes produces a spurious degree-$(23,24)$ function
that fails on held-out nodes). The poles of $j$ are the multiplicative fibres,
of type $I_n$ with $n=\operatorname{ord}_t$ of the pole; $D$ factors with pole
multiplicities $(6,4,4,2,1,1,1,1,1,1,1,1)$, giving the stated list. As
$\sum e(F_v)=24=e(Y)$ is already exhausted by these fibres, no additive fibre
can occur (each would contribute $e\ge2$, and an $I_n^{*}$ at a simple pole would
contribute $e\ge7$). Consistently, $N$ vanishes to order $3$ at every zero of
$j$ and $N-1728\,D$ to order $2$ at every point where $j=1728$ --- the
ramification a genuine functional invariant must have --- and $t=0$ is not a
pole ($j(0)$ finite), matching the smooth reduced fibre of the double fibre
${}_2I_0$. The computation was carried out at $p=1009$ and independently at
$p=2003$, yielding the same partition (the $I_4$ pair and the eight $I_1$'s
being defined over quadratic, resp.\ quartic, residue extensions).
\end{proof}

\begin{remark}
The reduction is at split primes of good reduction for the fibration; the pole
multiplicities are the $I_n$ indices and are preserved. The Shioda--Tate
relation is consistent: the reducible fibres contribute
