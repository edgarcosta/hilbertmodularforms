...
intrinsic HilbertCuspForms(Mk::ModFrmHilD) -> ModFrmHil
  ...
  if not assigned Mk`MagmaSpace then
    ...
    // Call without character first to avoid type issues, then apply character subspace
...
intrinsic DefaultCoefficientRing(Mk::ModFrmHilD) -> FldNum
  {
    ...
    returns:
      ...
      In nonparallel paritious weight, or in nonparitious
      weight when the narrow class number of F is 1, this is
      ...
      If k is non-paritious and the narrow class number is 
      not 1, we return the compositum
      of the nebentypus field, the splitting field of F and 
