...
function TotallyPositiveUnitsModSquaresRepresentatives(UF)
...
function OrderWithConductor(ZK, ff)
    ...
    //ZK := MaximalOrder(Order(ff));
    K  := NumberField(ZK);

    Oq := sub<ZK | [K ! g * zki * ZK.2 : g in Generators(ff),
...
function ClassNumberOfOrderWithConductor(ZK, bb)
    // Class number of the order in ZK/ZF with conductor bb.
...
function Product(L : empty:=1)
    // Return the product of the elements of L, with empty product given by `empty`.
    if IsEmpty(L) then
        return empty;
    else
        return &*L;
...
// Record type to store the results of this output.
OrderTermRecordFormat := recformat<Order, Conductor, PicardNumber, HasseUnitIndex>;

intrinsic OrderTermData(F::FldNum, rho : Bound:=0) -> Rec
...
as a record.}
    ...
    return OrderTermData(S, K, ZK, ZF, hK);
end intrinsic;
    
intrinsic OrderTermData(S::RngOrd, K::FldNum, ZK::RngOrd, ZF::RngOrd, hK::RngIntElt) -> Rec
...
{Given a congruence subgroup `Gamma` (level, field, decoration data), return
    ...
    for rho in Sdiv do
        ...
        listOfOrders := OrderTermData(F, rho);
        count := 0;
