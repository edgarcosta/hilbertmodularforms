...
intrinsic FindNewGenerators(Mk::ModFrmHilD, EvaluatedMonomials::SeqEnum, BasisWeightK::SeqEnum : Alg := "WeightedLLL")-> Any
...
- Relations takes as input the associtive arrays found in ConstructGeneratorsAndRelations
  It then checks for further relations (NOT GENERATORS!) in higher weights. It doesn't call Basis(M) so it runs extremely quickly.
...
FIXME: refactor ConstructGeneratorsAndRelations, ConstructGeneratorsAndRelationsOnComponent, ConstructGeneratorsAndRelationsSymmetric to avoid copy paste code!
...
// Input: MaxWeight = highest weight to check for relations in
// Output: Associative array of generators in each weight, Associative array of Relations in each weight and Quotient Ring
intrinsic ConstructGeneratorsAndRelationsOnComponent(M::ModFrmHilDGRng, N::RngOrdIdl, MaxWeight::RngIntElt, bb::RngOrdIdl) -> Any
	{Finds all Generators and Relations}
	...
	if not IsNull(LowestWeightBasis) then
		...
		// CONFUSED: before we printed Weight = 2
		vprintf HilbertModularForms : "Weight: %o     Generarators: %o Relations: %o\n", LowestWeight, #Gens[LowestWeight],  0;
...
intrinsic ConstructGeneratorsAndRelationsSymmetric(M::ModFrmHilDGRng, N::RngOrdIdl, MaxWeight::RngIntElt) -> Any
	{Finds all Generators and Relations}
